...
class Listener:
    ...
    #Subscribes on the topic \motor_twist and sets the motors appropriately based on the received Twist message
    def __init__(self):
	...
	rospy.loginfo("R_motor set to: " + str(self.setRight))
        ...
        #Publish on 'motor_speeds' topic
        pub = rospy.Publisher('motor_speeds', MotorSpeedsStamped, queue_size=10)
        message = MotorSpeedsStamped()
        message.header.stamp = rospy.Time.now()
        message.left = self.setLeft
        message.right = self.setRight
        pub.publish(message)
